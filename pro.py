...
# ==============================
# 3. BỘ LỌC CỔ PHIẾU NÓNG & TELEGRAM
# ==============================
st.markdown("---")
with st.expander("🔥 TÌM KIẾM CỔ PHIẾU NÓNG CHO LƯỚT SÓNG (Mở rộng để xem)", expanded=True):
    ...
    if not top_stocks.empty:
        st.dataframe(top_stocks.style.format({"Giá Hiện Tại": "{:,.2f}", "Biên độ (%)": "{:.2f}%", "Thanh khoản TB": "{:,}", "Volume Đột biến": "x{:.1f}"}), use_container_width=True, hide_index=True)
        
        best = top_stocks.iloc[0]
        st.success(f"⚡ **Mã lướt biên lớn nhất:** **{best['Mã CP']}** (Biên độ trung bình **{best['Biên độ (%)']}%**/ngày).")
        
        # LOGIC TELEGRAM (Chỉ gửi 1 lần mỗi phiên để tránh spam)
        if "telegram_sent" not in st.session_state:
            if float(best['Biên độ (%)']) >= 3.0:
                msg = f"🚀 [PRO TERMINAL - BÁO ĐỘNG]\nPhát hiện siêu phẩm lướt sóng: *{best['Mã CP']}*\n- Giá: {best['Giá Hiện Tại']}\n- Biên độ: {best['Biên độ (%)']}%\n- Volume TB: {best['Thanh khoản TB']:,} cổ"
